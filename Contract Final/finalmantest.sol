/**
 *Submitted for verification at BscScan.com on 2024-02-13
*/
...
contract ManaTee is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    ManaTeeDividendTracker public dividendTracker;
    ...
    //staking variables
    mapping(address => uint256) public tokensStaked14Days;
    mapping(address => uint256) public tokensStaked30Days;
    mapping(address => uint256) public tokensStaked90Days;
    mapping(address => bool) public _isStaked14Days;
    mapping(address => bool) public _isStaked30Days;
    mapping(address => bool) public _isStaked90Days;
    mapping(address => uint256) public _staked14DaysTimestamp;
    mapping(address => uint256) public _staked30DaysTimestamp;
    mapping(address => uint256) public _staked90DaysTimestamp;
    ...
    event UpdateFees(uint256 sellDeadFees, uint256 sellRewardsFee,
                     uint256 buyDeadFees, uint256 buyRewardsFee);
    ...
    constructor() ERC20("Manatee", "Manatee") {
        address router = 0x581fA0Ee5A68a1Fe7c8Ad1Eb2bfdD9cF66d3d923;
        ...
        dividendTracker = new ManaTeeDividendTracker(payable(this), router, 0xf60264E9793DABEdD2Ca0f0E7D4974D3203cd8A0,
                                                      "ManateeTRACKER", "ManateeTRACKER");
    ...
    function enableTrading(uint256 initialMaxGwei, uint256 initialMaxWallet, uint256 initialMaxTX,
        ...
        require(initialMaxGwei >= 3, "cannot set below 5");
    ...
    function updateFees(uint256 deadBuy, uint256 deadSell,
        ...
        require(deadBuy <= 400 && deadSell <= 400, "dead fees cannot exceed 4%");
        require(totalSellFees <= 800 && totalBuyFees <= 800, "total fees cannot exceed 8% sell or buy");

        emit UpdateFees(sellDeadFees, buyDeadFees,
                        buyRewardsFee, sellRewardsFee);
    }
    ...
    function StakeFor14Days(uint256 amount) public {
        amount = amount * 10**18;
        tokensStaked14Days[msg.sender] = tokensStaked14Days[msg.sender] + amount;

        if (_isStaked14Days[msg.sender]) {
            dividendTracker.setBalance(msg.sender, getStakingBalance(msg.sender));
        }
        else {
             _isStaked14Days[msg.sender] = true;
            _staked14DaysTimestamp[msg.sender] = block.timestamp;
            dividendTracker.setBalance(msg.sender, getStakingBalance(msg.sender));
        }
    }

    function StakeFor30Days(uint256 amount) public {
        amount = amount * 10**18;
        tokensStaked30Days[msg.sender] = tokensStaked30Days[msg.sender] + amount;

        if (_isStaked30Days[msg.sender]) {
            dividendTracker.setBalance(msg.sender, getStakingBalance(msg.sender));
        }
        else {
             _isStaked30Days[msg.sender] = true;
            _staked30DaysTimestamp[msg.sender] = block.timestamp;
            dividendTracker.setBalance(msg.sender, getStakingBalance(msg.sender));
        }
    }

    function StakeFor90Days(uint256 amount) public {
        amount = amount * 10**18;
        tokensStaked90Days[msg.sender] = tokensStaked90Days[msg.sender] + amount;
        if (_isStaked90Days[msg.sender]) {
            dividendTracker.setBalance(msg.sender, getStakingBalance(msg.sender));
        }
        else {
            _isStaked90Days[msg.sender] = true;
            _staked90DaysTimestamp[msg.sender] = block.timestamp;
            dividendTracker.setBalance(msg.sender, getStakingBalance(msg.sender));
        }

    }

    function Unstake14Days() public {
        require(_isStaked14Days[msg.sender] && block.timestamp >= _staked14DaysTimestamp[msg.sender] + 1 days,
        "14 days staking has not ended");

        _isStaked14Days[msg.sender] = false;
        _staked14DaysTimestamp[msg.sender] = 0;
        tokensStaked14Days[msg.sender] = 0;
        dividendTracker.setBalance(msg.sender, getStakingBalance(msg.sender));

    }

    function Unstake30Days() public {
        require(_isStaked30Days[msg.sender] && block.timestamp >= _staked30DaysTimestamp[msg.sender] + 2 days,
        "30 days staking has not ended");

        _isStaked30Days[msg.sender] = false;
        _staked30DaysTimestamp[msg.sender] = 0;
        tokensStaked30Days[msg.sender] = 0;
        dividendTracker.setBalance(msg.sender, getStakingBalance(msg.sender));

    }

    function Unstake90Days() public {
        require(_isStaked90Days[msg.sender] && block.timestamp >= _staked90DaysTimestamp[msg.sender] + 3 days,
        "90 days staking has not ended");

        _isStaked90Days[msg.sender] = false;
        _staked90DaysTimestamp[msg.sender] = 0;
        tokensStaked90Days[msg.sender] = 0;
        dividendTracker.setBalance(msg.sender, getStakingBalance(msg.sender));
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (_isStaked14Days[from] || _isStaked30Days[from] || _isStaked90Days[from]) {
                    require(balanceOf(from) - amount >= tokensStaked14Days[from] + tokensStaked30Days[from]
                            + tokensStaked90Days[from],"tokens are staked");
        }
    ...
    function getStakingBalance(address account) private view returns (uint256) {
        uint256 stakedBalance14Days;
        uint256 stakedBalance30Days;
        uint256 stakedBalance90Days;
        uint256 stakingBalance;
        uint256 unstakedBalance;


        if (_isStaked14Days[account]) {
            stakedBalance14Days = tokensStaked14Days[account] * 2;
        }
        if (_isStaked30Days[account]) {
            stakedBalance30Days = tokensStaked30Days[account] * 3;
        }
        if (_isStaked90Days[account]) {
            stakedBalance90Days = tokensStaked90Days[account] * 9;
        }

        unstakedBalance = balanceOf(account).sub(tokensStaked14Days[account] + tokensStaked30Days[account]
                          + tokensStaked90Days[account]);

        stakingBalance = unstakedBalance.add(stakedBalance14Days + stakedBalance30Days + stakedBalance90Days);
    ...
    function setDividendTracker(address NewAddress) public onlyOwner {
        dividendTracker = ManaTeeDividendTracker(payable(NewAddress));
    }
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        if (buyRewardsFee > 0) {
            ...
            .mul(buyRewardsFee).div(buyRewardsFee);
        }
        if (sellRewardsFee > 0) {
            dividendsFromSell = address(this).balance.mul(sellAmount).div(totalAmount)
            .mul(sellRewardsFee).div(sellRewardsFee);
        }
...
contract ManaTeeDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    ManaTee public ManaTeeContract;
    ...
    constructor(address payable mainContract, address router, address token, string memory _name,
        ...
        defaultToken = token;
        ManaTeeContract = ManaTee(mainContract);
    ...
    function withdrawDividend() public pure override {
        require(false, "withdrawDividend disabled. Use the 'claim' function on the main ManaTee contract.");
    }
    ...
    function _reinvestDividendOfUser(address account) private returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(ManaTeeContract);

            uint256 prevBalance = ManaTeeContract.balanceOf(address(this));
            ...
            {
                uint256 received = ManaTeeContract.balanceOf(address(this)).sub(prevBalance);
                if (received > 0) {
                    success = true;
                    ManaTeeContract.transfer(account, received);
                }
                else {
                    success = false;
                }
